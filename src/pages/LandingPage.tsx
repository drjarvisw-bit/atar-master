    ...
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-5xl px-6 pt-24 pb-20 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              to="/skill-tree"
              className="inline-flex items-center gap-2 bg-gh-accent-blue-bold hover:bg-gh-accent-blue text-white font-semibold px-8 py-3.5 rounded-xl transition-all hover:scale-105 hover:shadow-lg hover:shadow-gh-accent-blue-bold/25"
            >
              <GitBranch size={18} />
              Start Learning Free
            </Link>
      ...
      <section className="border-y border-gh-border bg-gh-surface/50">
        <div className="mx-auto max-w-4xl px-6 py-12 grid grid-cols-3 gap-8 text-center">
          {stats.map((s) => (
            <div key={s.label}>
              <div className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-gh-accent-blue to-purple-400 bg-clip-text text-transparent">
                {s.value}
              </div>
      ...
      <section id="features" className="mx-auto max-w-5xl px-6 py-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {features.map((f) => (
            <div
              key={f.title}
              className={`group relative border border-gh-border rounded-2xl p-8 bg-gh-surface transition-all duration-300 ${f.border} hover:-translate-y-1 hover:shadow-xl ${f.glow}`}
            >
              <div className="text-4xl mb-4">{f.emoji}</div>
              <h3 className={`text-xl font-bold mb-3 bg-gradient-to-r ${f.color} bg-clip-text text-transparent`}>{f.title}</h3>
          ...
          {plans.map((plan) => (
            <div
              ...
              className={`relative rounded-2xl p-8 border transition-all ${
                plan.highlight
                  ? 'border-gh-accent-blue-bold bg-gradient-to-b from-gh-accent-blue-bold/10 to-gh-surface shadow-lg shadow-gh-accent-blue-bold/10'
                  : 'border-gh-border bg-gh-surface'
              }`}
            >
              {plan.highlight && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-gh-accent-blue-bold text-xs font-semibold text-white">
                  MOST POPULAR
                </div>
              )}
              ...
              <Link
                ...
                className={`block w-full text-center py-3 rounded-xl font-semibold transition-all ${
                  plan.highlight
                    ? 'bg-gh-accent-blue-bold hover:bg-gh-accent-blue text-white hover:shadow-lg hover:shadow-gh-accent-blue-bold/25'
